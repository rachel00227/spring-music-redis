angular.module('albums', ['ngResource', 'ui.bootstrap']).
    ...
    factory('SendList', function ($resource) {
        return $resource('list/send');
    }).
    factory('GetList', function ($resource) {
        return $resource('list/get');
    }).
...
function AlbumsController($scope, $modal, Albums, Album, GetList, Status, SendList) {
    ...
    function saveList(albums) {
    	SendList.save(albums,
            function () {
                Status.success("List saved");
            },
            function (result) {
                Status.error("Error saving album: " + result.status);
            }
...
function ListController($scope,$modalInstance, GetList,action) {
    $scope.albumAction = action;
    $scope.albums = GetList.query();
